...
一句话说明怎么改才能符合上述法律要求。"""
        ...
        user_content = f"""改写以下隐私政策条款。

【原条款】
{request.original_snippet}

【合规参考材料 — 仅用于理解合规要求，绝对不可出现在输出中】
{legal_content}

{edit_instruction}

【要求】
- 直接输出改写后的条款正文，不要任何前缀、解释、标注
- 用最少的字把该说的合规要点都说清楚——每句话必须有实质信息，没有一个废字
- 输出中禁止出现：任何法律名称、任何条文编号、任何条文原文或转述、"根据XX法"等引用性表述
- 上方【合规参考材料】中的法律内容仅作为改写依据，理解后内化为条款措辞，不得外显
...
请改写上面的原条款："""
        ...
        system_content = ("你是资深隐私政策合规专家。改写标准：用最简单的语言写出最合规的条款——字少、事全、无废话。"
            "你会收到完整的法律条文作为参考材料，请深入理解其合规要求后，将法律精神内化为精准的条款措辞。"
            "输出中绝不可出现任何法律名称、条文编号、条文原文或引用性表述——法律知识只能体现在改写质量上，不能体现在文字表面。"
            "只输出改写正文。")
