...
一句话说明怎么改才能符合上述法律要求。"""
        ...
        EDIT_INSTRUCTIONS = {
            "I1": "删除或缩小敏感数据收集范围。只保留与核心功能直接相关的信息类型，加上'仅限于用户主动使用该功能时'的限定语。将笼统的'可能收集'改为明确的'仅在[某功能]中使用时收集[某类信息]'格式。",
            "I2": "在每项信息收集后面补上具体用途说明，格式为'用于[具体功能]'。删除'提升体验''业务需要''优化服务'等所有模糊表述，替换为具体场景。",
            "I3": "把默认同意改为主动选择。加上'您可以选择是否'前缀，删除'即视为同意''注册即表示''使用本服务即视为'等所有捆绑授权表述。改为独立的勾选框+确认按钮流程。",
            "I4": "删除与服务核心功能无关的信息收集项。保留的每项都要能回答'这个功能为什么需要它'。用'仅限以下场景'列表替代开放式列举。",
            "I5": "把'合作伙伴''第三方'等模糊称谓替换为具体类型（如'支付服务商''物流配送方''数据分析服务商'），并列出每种情况共享的具体数据种类和目的。",
            "I6": "在第三方共享描述前加上'我们将单独征得您的明确同意后'，作为独立于主政策的二次确认步骤。不能用主政策的一次性同意覆盖。",
            "I7": "在第三方共享描述后补充不可逾越的目的限制，如'接收方仅可将数据用于[指定用途]，不得转售、不得用于其自身营销或其他未获授权之目的'。",
            "I8": "为每类数据分别添加明确留存时限，格式为'保存期限为[具体时间]，届满后将自动删除或匿名化处理'。禁止使用'必要期间''合理期限'等无期限表述。",
            "I9": "补充具体的销毁方式说明：'保存期限届满后，我们将通过[具体技术手段，如安全擦除/匿名化]使数据无法恢复'。不能只说'我们会删除'而不说怎么做。",
            "I10": "完整列举用户可行使的权利项目：查询自己的信息、要求更正错误信息、要求删除、撤回已给予的同意、获取数据可携带副本、对处理方式提出投诉举报。每项都要有对应的操作入口说明。",
            "I11": "提供至少两种具体联系方式：应用内的在线入口路径（如'设置→隐私→意见反馈'）、公开的邮箱地址或客服热线号码。加上'我们将在15个工作日内响应'的时限承诺。",
            "I12": "加上具体时限承诺：'我们将在收到您的书面请求后15个工作日内完成处理，并以您提供的联系方式反馈结果'。不能只说'尽快处理'。",
        }
        edit_instruction = EDIT_INSTRUCTIONS.get(request.violation_type,
            "修改条款使其符合合规要求：删除模糊表述、缩小收集范围、增加用户选择权、明确期限和联系方式。")

        user_content = f"""请根据下方的【修改指令】，对原条款进行针对性改写。
...
【改写要求 — 必须严格遵守】
1. 直接输出改写后的完整文字，不要加任何前缀、后缀、解释或标注
2. 输出内容必须是一段专业、简洁、可直接替换原条款的合规文案
3. 禁止出现以下内容（违反任一条即为不合格输出）：
   × 任何法律名称（如"个保法""网络安全法""民法典""GDPR"）
   × 任何条目编号（如"第六条""第41条""第17条"）
   × 任何法律条文原文或转述（如"应当取得单独同意""遵循最小必要原则"）
   × 任何"根据XX法""依据XX规定""符合XX条要求"之类的引用性表述
   × 对法律条文的复述、摘录或注释
4. 文风要求：专业、严谨、简洁。用正式的隐私政策措辞风格，每句话都要有明确的合规含义

【参考范例】

原句：「为了提供更好的服务，我们可能会收集您的位置信息、通讯录等。」
改写：「我们仅在用户主动使用特定功能时收集必要的设备与位置信息：(1)地图导航功能——用于路径规划与实时定位；(2)消息功能——用于发送文件与建立连接。用户可在「设置-隐私」中随时管理或撤销上述授权。」

原句：「注册即视为您已同意本政策全部条款。」
改写：「完成注册前，请您审阅本政策全部内容并主动勾选确认框以表示知情同意。该同意可由用户在账户设置中随时撤回，不影响撤回前基于同意所进行的数据处理的合法性。」

现在请改写上面的原条款："""

        system_content = ("你是资深隐私政策合规专家，擅长撰写符合中国个人信息保护法规的专业条款文案。"
            "你的改写必须同时满足三个标准：(1)专业——使用标准的隐私政策正式措辞；(2)简洁——去除冗余和模糊表述，每句话都有实质信息；(3)合规——隐含满足相关法律要求但不引用法律条文。"
            "绝对禁止在输出中出现任何法律名称、条文编号、法律原文、'根据XX法'等引用性表述，也不要对法律条文进行转述或复述。"
            "将法律要求内化为具体的条款表述，而非外显为法律引用。"
            "只输出改写后的条款正文，其他一个字都不要多写。")
