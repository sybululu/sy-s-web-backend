...
2. 输出内容必须是一段可以直接替换原条款的干净文案
3. 禁止出现以下内容（违反任一条即为不合格输出）：
   × 任何法律名称（如"个保法""网络安全法""民法典""GDPR"）
   × 任何条目编号（如"第六条""第41条""第17条"）
   × 任何法律条文原文或转述（如"应当取得单独同意""遵循最小必要原则"）
   × 任何"根据XX法""依据XX规定""符合XX条要求"之类的表述
   × 任何法律术语堆砌（如"知情同意""最小必要""目的限定""存储限制"）
4. 用普通用户能看懂的大白话写，像产品说明书而不是法律文件

【参考范例】

原句：「为了提供更好的服务，我们可能会收集您的位置信息、通讯录等。」
改写：「在使用地图导航功能时，我们会收集您的位置信息，用于实时导航和路线规划。您可以随时在设置中关闭此功能。」

原句：「注册即视为您已同意本政策全部条款。」
改写：「请您仔细阅读本政策全文。完成注册前，请勾选"我已阅读并同意"选项。您可以随时在账户设置中查看或撤回此项同意。」

现在请改写上面的原条款："""

        system_content = ("你是一个专业的隐私政策文案改写员。你的任务只有一个：把给定的条款按照修改指令改写成更合规、更清晰的版本。"
            "你输出的每一句话都必须是面向用户的说明性文字，不是法律分析。"
            "绝对禁止在你的输出中出现任何法律名称、条文编号、法律原文、法律术语或'根据XX法'这类表述。"
            "如果改写后的内容不可避免地涉及法律概念，用大白话翻译它（例如不说'遵循最小必要原则'，而说'我们只在需要的时候收集必要的信息'）。"
            "只输出改写结果，其他一个字都不要多写。")
