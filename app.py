...
请严格按上方 Output Format 输出，只输出两部分正文内容，不要添加任何额外文字、前缀、格式符号或分隔线。"""
        ...
        # ====== 改写模式：精准删改，简洁合规 ======
        # 设计原则：
        #   1. 模型角色是「产品文案编辑」非法务官——输出应像真实互联网产品的隐私政策，不是法律论文
        #   2. 核心操作是三步：删掉不合理的 → 修改模糊的 → 补上缺失的
        #   3. 输出必须比原文更短或等长，禁止展开论述
        #   4. 禁止复述/翻译法律条文，只输出改写后的最终文本
        violation_type_name = ID_TO_INDICATOR.get(request.violation_type, request.violation_type)
        violation_type_desc = f"{violation_type_name}（{request.violation_type}）"

        user_content = f"""# 你的任务
你是一位顶级互联网产品的文案编辑，专门负责把隐私政策改写得**简洁、自然、合规**。
你的工作不是写法律文书，而是让用户能看懂、愿意读、同时经得起监管检查。
...
## 第一步：删掉不合理的
- 删除与提供服务**无关**的信息收集项（如身份证号、银行卡号、性别、年龄等，除非服务确实需要）
- 删除自相矛盾的表述（如一边说"最少必要"，一边列一堆不必要的数据）
- 删除空洞的套话和重复内容（如"我们致力于...""为了尊重您的隐私..."等无信息量的句子）
- 删除已经包含在后续条款中的重复说明

## 第二步：修改模糊的
- 把"必要的个人信息"改为具体列出**真正需要的**项目（如"账号名、手机号、邮箱"）
- 把"优化服务体验"等模糊目的改为**具体场景**（如"完成订单、身份验证、客服联系"）
- 把"我们将通过显著方式告知"改为**用户能感知的具体动作**

## 第三步：补上缺失的（仅补关键缺失，不要展开论述）
- 如果缺少收集目的 → 用半句话点明用途
- 如果缺少同意机制 → 加一句"需您主动同意"
- 如果缺少权利说明 → 加一句"您可以随时查阅、更正或删除"
- **注意：只补一句，不要写成一段**

# 格式要求
1. **只输出改写后的最终文本**，不要任何解释、标注、前言
2. **篇幅控制**：输出长度不超过原文的 80%，越短越好
3. **语气要求**：像微信/支付宝/抖音隐私政策那样平实、直接，不要法务腔
4. **禁止事项**：
   - 禁止出现"根据XX法""依照XX规定""符合XX要求"等法律引用语
   - 禁止出现"我们致力于""我们承诺""我们高度重视"等空话套话
   - 禁止把上面的【合规依据】翻译或改写后塞进输出
   - 禁止用"首先...其次...最后..."等排比结构"""

        system_content = "你是产品文案编辑。直接输出改写后的隐私政策文本，要简洁、自然、合规。不要解释，不要引用法律。"
